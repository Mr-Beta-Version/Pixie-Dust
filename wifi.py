...
print("\n \t[>] Alway turn on hotspot on your phone before running this script \n")
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        if self.vuln_list:
            print('\n\nType: {1} {0} {2} {0} {3}'.format(
                '|',
                colored('VULNERABLE', color='green'),
                colored('LOCKED', color='red'),
                colored('HACKED', color='yellow')
            ))
        print('\n Available WIFI:\n')
        ...
        for n, network in network_list_items:
            ...
            line = f"{number:<4}" +', '.join([str(network[k]) for k in ['ESSID','Security type','WPS','WPS locked','Model','Model number','Device name','BSSID','Level']])
...
def usage():
    return """
OneShotPin 0.0.2 (c) 2017 rofl0r, modded by Mr. Beta
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='OneShotPin 0.0.2 (c) 2017 rofl0r, modded by Mr. Beta',
